...
var BtnGroup = function () {
  function BtnGroup(group) {
    ...
    this.slideContainer.style.width = ((this.slides.length+0)*70 + 'vw');
	//setTimeout(alert, 2000, "KK");
  ...
  {
	  key:'loadd',
	  value: function loadd(e){
		  var index = this.buttons.indexOf(e.srcElement || e.target);
		  //alert(index);
		  if(index<0){index=0};
			var _self = this;
			   var interval = setInterval(function() {
				if(index != _self.buttons.length-1){
					_self.activateIndex(++index);
				}
				else{
					index=-1;
				}
			   }, 3500);
			for (var _iterator = this.buttons[Symbol.iterator](), _stepp; !(_iteratorNormalCompletion = (_stepp = _iterator.next()).done); _iteratorNormalCompletion = true) {
				var button = _stepp.value;
					button.addEventListener('click', function (e) {
  ...
  },
  {
    ...
    value: function activateIndex(index) {
      this.slideContainer.style.left = ((-index)+0)*68 + 'vw';
